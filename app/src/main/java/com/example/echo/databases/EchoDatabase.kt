        ...
        try{
            ...
            if(cSor.moveToFirst()) {
                do{
                    ...
                    var _artist = cSor.getInt(cSor.getColumnIndexOrThrow(EchoDatabase.Statified.COLUMN_SONG_ARTIST))
                    var _title = cSor.getInt(cSor.getColumnIndexOrThrow(EchoDatabase.Statified.COLUMN_SONG_TITLE))
                    var _songPath = cSor.getInt(cSor.getColumnIndexOrThrow(EchoDatabase.Statified.COLUMN_SONG_PATH))
                    _songsList.add(Songs(_id.toLong(), _title.toString(), _artist.toString(), _songPath.toString(), 0))
